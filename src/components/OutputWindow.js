...
function OutputWindow(props) {
    ...
    if (simulationResult && simulationResult.error) {
        ...
        return <pre className="text-info  border border-secondary" style={{ textAlign: 'left', overflowY: "scroll", width: (vw * 99 / 100 - props.editorWidth) + "px", marginTop: '10px', height: vh * 85 / 100 + "px" }} role="output">{JSON.stringify(simulationResult.result.evaluationResult.func({ a: false, b: false }), null, 2)}</pre>
    }
